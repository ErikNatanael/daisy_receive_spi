...
void loop() {
  ...
  while (true) {
    ...
    if (reply == "audio") {
      ...
      if (audio_buffer_size != bytes_to_read) {
        Serial.printf("Mismatch between requested bytes %u and received %u\n",
...
void send_audio_buffer() {
  ...
  uint32_t d = spi_transfer_u32(audio_buffer_size);

  uint32_t buffer_size_ack = spi_transfer_u32(256);
  if (audio_buffer_size != buffer_size_ack) {
